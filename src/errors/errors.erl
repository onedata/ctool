...
-define(ERROR_ID_TO_TYPE_MAPPING, #{
    ...
    ?ERROR_BAD_VALUE_LIST_OF_STRINGS_ID => ?ERROR_BAD_VALUE_LIST_OF_STRINGS_TYPE,
    ...
    ?ERROR_BAD_VALUE_STRING_ID => ?ERROR_BAD_VALUE_STRING_TYPE,
...
to_json(?ERROR_UNRECOGNIZED_ERROR_MATCH(ErrorAsJson)) ->
...
to_json(Error = #od_error{type = Type}) ->
    Type:to_json(Error);
...
to_json(OtherError) ->
    ...
    % unexpected exception (an ?new_ERROR_INTERNAL_SERVER_ERROR(ErrorRef) is returned).
...
from_json(ErrorJson) ->
    try
        ...
        ?new_ERROR_UNRECOGNIZED_ERROR(ErrorJson)
    end.


-spec to_http_code(error()) -> 400 | 401 | 403 | 404 | 409 | 500 | 501 | 503.
to_http_code(?ERROR_UNRECOGNIZED_ERROR_MATCH(_)) -> 
...
to_errno(?ERROR_UNRECOGNIZED_ERROR_MATCH(_)) -> 
    {true, ?EAGAIN};

to_errno(Error = #od_error{type = Type}) ->
    Type:to_errno(Error).
