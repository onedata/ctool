...
to_json(?ERR_ATM_STORE_CREATION_FAILED(ErrorCtx, AtmStoreSchemaId, SpecificError)) ->
    SpecificErrorJson = errors:to_json(SpecificError),
    SpecificErrorPrint = maps:get(<<"description">>, SpecificErrorJson),

    #{
        <<"id">> => ?ERR_ATM_STORE_CREATION_FAILED_ID,
        ...
        <<"description">> => od_error:format_description(
            "Failed to create an automation store (schema ID: \"~ts\"): ~ts",
...
from_json(OdErrorJson = #{<<"id">> := ?ERR_ATM_STORE_CREATION_FAILED_ID}) ->
    ...
    ?ERR_ATM_STORE_CREATION_FAILED(ErrorCtx, AtmStoreSchemaId, SpecificError).
