...
to_json(?ERR_ATM_LAMBDA_CONFIG_BAD_VALUE(ErrorCtx, ParameterName, SpecificError)) ->
    SpecificErrorJson = errors:to_json(SpecificError),
    SpecificErrorPrint = maps:get(<<"description">>, SpecificErrorJson),

    #{
        <<"id">> => ?ERR_ATM_LAMBDA_CONFIG_BAD_VALUE_ID,
        ...
        <<"description">> => od_error:format_description(
            "Bad value provided for parameter \"~ts\" of lambda config: ~ts",
            [ParameterName, SpecificErrorPrint]
        )
    }.


-spec from_json(json_utils:json_map()) -> t().
from_json(OdErrorJson = #{<<"id">> := ?ERR_ATM_LAMBDA_CONFIG_BAD_VALUE_ID}) ->
    ...
    ?ERR_ATM_LAMBDA_CONFIG_BAD_VALUE(ErrorCtx, ParameterName, SpecificError).
