...
to_json(?ERR_ATM_TASK_EXECUTION_INITIATION_FAILED(ErrorCtx, AtmTaskSchemaId, SpecificError)) ->
    SpecificErrorJson = errors:to_json(SpecificError),
    SpecificErrorPrint = maps:get(<<"description">>, SpecificErrorJson),

    #{
        <<"id">> => ?ERR_ATM_TASK_EXECUTION_INITIATION_FAILED_ID,
        ...
        <<"description">> => od_error:format_description(
            "Failed to initiate automation task execution (ID: \"~ts\"): ~ts",
...
from_json(OdErrorJson = #{<<"id">> := ?ERR_ATM_TASK_EXECUTION_INITIATION_FAILED_ID}) ->
    ...
    ?ERR_ATM_TASK_EXECUTION_INITIATION_FAILED(ErrorCtx, AtmTaskSchemaId, SpecificError).
