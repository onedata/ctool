...
%%% @end
%%%-------------------------------------------------------------------
-module(atm_time_series_data_type).
-author("Lukasz Opiola").
...
value_constraints_to_json(#{specs := Specs}) ->
    #{<<"specs">> => jsonable_record:list_to_json(Specs, atm_time_series_data_spec)}.
...
value_constraints_from_json(#{<<"specs">> := SpecsJson}) ->
    #{specs => jsonable_record:list_from_json(SpecsJson, atm_time_series_data_spec)}.
