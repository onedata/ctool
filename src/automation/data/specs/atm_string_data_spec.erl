...
%%%-------------------------------------------------------------------
%%% @doc
%%% Module implementing the string data spec used in automation machinery.
%%% @end
%%%-------------------------------------------------------------------
-module(atm_string_data_spec).
-author("Lukasz Opiola").
...
-type record() :: #atm_string_data_spec{}.
...
-spec encode_with(record(), persistent_record:nested_record_encoder()) ->
    json_utils:json_map().
encode_with(#atm_string_data_spec{allowed_values = AllowedValues}, _NestedRecordEncoder) ->
    #{
...
%% @private
-spec decode_allowed_values(null | [binary()], jsonable_record:validation_strategy()) ->
    undefined | [binary()] | no_return().
...
decode_allowed_values(Values, validate) ->
    case is_list(Values) andalso lists:all(fun is_binary/1, Values) of
        true -> Values;
        false -> throw(?ERR_BAD_DATA(?err_ctx(), <<"allowedValues">>, <<"You must provide a list of strings">>))
    end.
