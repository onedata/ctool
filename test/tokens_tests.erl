...
% make sure that the token serialization / deserialization is deterministic and
% does not change between system versions, despite changes in dependencies and
% Erlang OTP upgrades
backward_compatibility_test() ->
    ...
    ?assertEqual(
        ...
        >>},
        tokens:serialize(tokens:construct(
    ...
    AccessTokenWithSessionCaveats = lists:reverse(AccessTokenCaveats),
    ?assertEqual(
        {ok, <<
        ...
        >>},
        tokens:serialize(tokens:construct(
            AccessTokenWithSessionPrototype,
            <<"verySecretSecret2">>,
            AccessTokenWithSessionCaveats
        ))
    ),
