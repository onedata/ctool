...
eunit:
	ERL_FLAGS="-args_file test/conf/eunit.vm.args -config test/conf/eunit.config" ./rebar3 eunit skip_deps=true --suite=${SUITES}
...
codetag-tracker:
	@./bamboos/scripts/run-with-surefire-report.py \
		...
		./bamboos/scripts/codetag-tracker.sh --branch=${BRANCH} --excluded-dirs=codetag_tracker_results
